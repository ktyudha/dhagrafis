...
const store = async (values, actions) => {
  ...
  await ApiService({ requiresAuth: true })
    .post(`admin/keunggulan/info/edit`, formData.value, {
      headers: {
...
onBeforeMount(async () => {
  const katalog_info = await ApiService({ requiresAuth: true })
    .get('admin/keunggulan/info')
    .then((res) => res.data.katalog)

  formData.value = katalog_info
  imagePreview.value = katalog_info.image ? [katalog_info.image] : []
...
<template>
  <h1 class="mb-8 text-3xl font-bold">Katalog Info</h1>
