...
const store = async (values, actions) => {
  ...
  await ApiService({ requiresAuth: true })
    .post(`admin/articles/${formData.value.slug}`, formData.value, {
      headers: {
    ...
    .then((res) => {
      Alert.fire({
        ...
        router.push({ name: 'articles.edit', params: { slug: res.data.article.id } })
      })
    })
    .catch((res) => {
...
onBeforeMount(async () => {
  const article = await ApiService({ requiresAuth: true })
    .get('admin/articles/' + route.params.slug)
    .then((res) => res.data.article)

  formData.value = article
  formData.value.category_id = article.category.id
  image.value = article.image ? [article.image] : []
