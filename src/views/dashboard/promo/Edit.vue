...
const store = async (values, actions) => {
  ...
  await ApiService({ requiresAuth: true })
    .post(`gallery/${formData.value.id}`, formData.value, {
      headers: {
...
onBeforeMount(async () => {
  const promo = await ApiService({ requiresAuth: true })
    .get('gallery/' + route.params.id)
    .then((res) => res.data.promo)

  formData.value = promo
  imagePreview.value = promo.image ? [promo.image] : []
})
</script>

<template>
  <h1 class="mb-8 text-3xl font-bold">Promo</h1>
