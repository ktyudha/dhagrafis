...
const store = async (values, actions) => {
  ...
  await ApiService({ requiresAuth: true })
    .post(`admin/contact/info/edit`, formData.value, {
      headers: {
...
onBeforeMount(async () => {
  const contact_info = await ApiService({ requiresAuth: true })
    .get('admin/contact/info')
    .then((res) => res.data.contact)

  formData.value = contact_info
  imagePreview.value = contact_info.image ? [contact_info.image] : []
  ...
  <div class="card bg-base-100 shadow">
    <div class="card-body">
      <Form :validation-schema="formSchema" @submit="store">
        ...
        <div class="grid grid-cols-2 gap-7" v-if="formData.additional_info">
          <div class="form-control w-full">
            ...
            <Field
              type="text"
              name="additional_info[0]['phone']"
              id="additional_info[0]['phone']"
              placeholder="Enter your Phone"
              class="input input-bordered w-full"
              required
              v-model="formData.additional_info[0].phone"
            />
            <ErrorMessage name="title" class="mt-2 text-sm text-red-600 dark:text-red-400" />
          </div>
          ...
          <div class="form-control w-full">
            ...
            <Field
              type="text"
              name="additional_info[0]['email']"
              id="additional_info[0]['email']"
              placeholder="Enter your title"
              class="input input-bordered w-full"
              required
              v-model="formData.additional_info[0].email"
            />
            <ErrorMessage name="title" class="mt-2 text-sm text-red-600 dark:text-red-400" />
          </div>

          <div class="form-control w-full mb-6">
            <label class="label" for="additional_info[0]['address']">
              <span class="label-text text-base-content">Address</span>
            </label>
            <Field
              as="textarea"
              rows="4"
              name="additional_info[0]['address']"
              id="additional_info[0]['address']"
              placeholder="Type your content here..."
              class="textarea textarea-bordered"
              required
              v-model="formData.additional_info[0].address"
            />
            <ErrorMessage
              name="additional_info[0]['address']"
          ...
          <div class="form-control w-full mb-6">
            <label class="label" for="additional_info[0]['gmap']">
              <span class="label-text text-base-content">Gmap</span>
            </label>
            <Field
              as="textarea"
              rows="4"
              name="additional_info[0]['gmap']"
              id="additional_info[0]['gmap']"
              placeholder="Type your content here..."
              class="textarea textarea-bordered"
              required
              v-model="formData.additional_info[0].gmap"
            />
            <ErrorMessage name="title" class="mt-2 text-sm text-red-600 dark:text-red-400" />
          </div>
        </div>
        ...
        <div class="form-control w-full mb-6" v-if="formData.additional_info">
          <label class="label" for="additional_info[0]['title']">
            <span class="label-text text-base-content">Title</span>
          </label>
          <Field
            type="text"
            name="additional_info[0]['title']"
            id="additional_info[0]['title']"
            placeholder="Enter your title"
            class="input input-bordered w-full"
            required
            v-model="formData.additional_info[0].title"
          />
          <ErrorMessage
            name="additional_info[0]['title']"
            class="mt-2 text-sm text-red-600 dark:text-red-400"
