...
const store = async (values, actions) => {
  ...
  await ApiService({ requiresAuth: true })
    .post(`admin/jobs/info/edit`, formData.value, {
      headers: {
...
onBeforeMount(async () => {
  const jobs_info = await ApiService({ requiresAuth: true })
    .get('admin/jobs/info')
    .then((res) => res.data.jobs)

  formData.value = jobs_info
  imagePreview.value = jobs_info.image ? [jobs_info.image] : []
