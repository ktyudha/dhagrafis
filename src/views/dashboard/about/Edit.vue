...
const store = async (values, actions) => {
  ...
  await ApiService({ requiresAuth: true })
    .post(`admin/about/${formData.value.slug}`, formData.value, {
      headers: {
    ...
    .then((res) => {
      Alert.fire({
        ...
        router.push({ name: 'about.edit', params: { slug: res.data.article.id } })
      })
    })
    .catch((res) => {
...
onBeforeMount(async () => {
  const about = await ApiService({ requiresAuth: true })
    .get('admin/about/' + route.params.slug)
    .then((res) => res.data.article)

  formData.value = about
  formData.value.category_id = about.category.id
  image.value = about.image ? [about.image] : []
...
<template>
  <h1 class="mb-8 text-3xl font-bold">Edit Articles</h1>
